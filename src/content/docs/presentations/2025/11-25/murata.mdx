...
- ほとんどドキュメントに書いてある。 ref : [アーキテクチャ設計](https://github.com/TakumiOkayasu/pre-omusubi/blob/main/docs/architecture.md)

## ファイル構成

- このように配置してもらうと、開発が楽になる。
- `omusubi-m5stack`はM5Stack向けに実装したディレクトリ。`git submodule`を使用せず、このように管理したい。
  - 理由は特にない。自分的にはこの方がうれしいかな？って思ったので。
...
- C++14 → C++17 への完全移行
  - 部分部分でC++14のコードが残っている
    - C++17に完全移行する予定ではあるが、SPAJAMの後に本腰を入れる感じかな？
